...
def _init_args():
    ...
    parser.add_argument("--test_model", action="store_true", help="Whether to test model logits")
    parser.add_argument("--test_conversion", action="store_true", help="Whether to test the conversion")
...
def convert_hf_to_mcore(hf_model_path, output_path, use_cpu_initialization=False, test_model=True, test_convert=False, trust_remote_code=False):
    os.makedirs(output_path, exist_ok=True)
    if len(os.listdir(output_path)) > 0 and not test_model:
    ...
    if test_model:
        ref_state_dict = model[0].module.sharded_state_dict()
        print("Loading mcore model")
        dist_checkpointing.load(ref_state_dict, output_path, strict=StrictHandling.ASSUME_OK_UNEXPECTED)
        megatron_model = model[0].cuda().module
        print(f"Loaded mcore model from {output_path}")
        from transformers import AutoModelForCausalLM, AutoModelForImageTextToText

        hf_model = AutoModelForCausalLM.from_pretrained(
            hf_model_path, torch_dtype=torch.bfloat16, trust_remote_code=trust_remote_code, device_map=0
        )
        print(f"Initialized Huggingface model from {hf_model_path}")
        bs = 1
        seqlen = 10
        input_ids = torch.randint(0, hf_config.vocab_size, (bs, seqlen), device="cuda")
        ...
        print(f"Generating {input_ids.shape=}")
        ref_output = hf_model.forward(input_ids, attention_mask=attention_mask, position_ids=position_ids)
        output = megatron_model(input_ids, position_ids, attention_mask)
...
if __name__ == "__main__":
    args = _init_args()
    convert_hf_to_mcore(
        args.hf_model_path, args.output_path, args.use_cpu_initialization, test_model=args.test_model, test_convert=args.test_conversion, trust_remote_code=args.trust_remote_code
    )
